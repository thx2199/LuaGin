-- detached,because cli can't use ngx.
require 'gin.core.detached'
-- gin
local Gin = require 'gin.core.gin'
local helpers = require 'gin.helpers.common'
local ins = require 'gin.helpers.inspect'
...
local function ensure_db_and_schema_migrations_exist(db, module_name)
...
function Migrations.version_already_run(db, version)
    local res = db:execute("SELECT version FROM " ..
        Migrations.migrations_table_name .. " WHERE version = '" .. version .. "';")
    return #res > 0
end
...
local function dump_schema_for(db)
    local schema_dump_file_path = Gin.app_dirs.schemas ..
        '/schemas.lua'
    local schema = db:schema()
    -- write to file
    local schema_str = ins.inspect(schema)
    if #schema_str > 2 then schema_str = schema_str:sub(2, -2) end -- remove first and last char
    local file_content = [[
local schema = {]] .. schema_str .. [[
}
return schema
    ]]
    local fw = io.open(schema_dump_file_path, "w")
...
local function run_migration(direction, module_name)
    ...
    -- print(json.encode(db.options))
    -- check adapter is supported
    if helpers.included_in_table(accepted_adapters, db.options.adapter) == false then
        local err_message = "Cannot run migrations for the adapter '" ..
            db.options.adapter .. "'. Supported adapters are: '" .. table.concat(accepted_adapters, "', '") .. "'."
        return false, version, err_message
    end

    if direction == "up" then ensure_db_and_schema_migrations_exist(db, module_name) end
